...
def bwd_expert_project_kernel_v4(
    ...
    dX_native_out, dExperts_out_fp32, # 🚨 接收 FP32 格式的影子权重梯度
    ...
    for curr_n in range(start_n, end_n, BLOCK_N):
        ...
        # 计算并写回 dX (因为有 sorted 隔离，同一时刻不同 Batch/Head 不会写同一 Token，极度安全)
    ...
    # 4. 循环结束：仅向 HBM 执行一次原子写入 (且为 FP32 格式，保精度)
    dw_ptrs = dExperts_out_fp32 + pid_m * stride_em + h_idx * stride_eh + offs_din[:, None] * stride_edin + offs_dout[None, :] * stride_edout
    tl.atomic_add(dw_ptrs, dw_acc, mask=mask_din[:, None] & mask_dout[None, :])
...
def triton_moe_project_backward_v4(X_native, dX_out_sorted, Experts, dX_native_out, sorted_indices, cu_seqlens, M, H):
    ...
    # 🚨 分配一块纯 FP32 的影子内存，隔离 FP16 的 Atomic Add
    dExperts_fp32 = torch.zeros(Experts.shape, dtype=torch.float32, device=Experts.device)
    
    BLOCK_DIN = triton.next_power_of_2(D_in_head)
    BLOCK_DOUT = triton.next_power_of_2(D_out_head)
    BLOCK_N = 64 # 回调到 64，因为不再占用 Grid，增大 Block 更利于 Tensor Core 吞吐

    # 2D Grid: M 决定专家并行，B*H 保证了足够跑满 GPU 的 384 个 Block
    grid = (M, B * H)
    
    bwd_expert_project_kernel_v4[grid](
        X_native, dX_out_sorted, Experts,
        dX_native_out, dExperts_fp32,
        sorted_indices, cu_seqlens,
    ...
    # 安全转回原格式
    return dExperts_fp32.to(Experts.dtype)
