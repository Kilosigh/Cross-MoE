...
# ==========================================
    ...
    # 2. 预热 (Warmup): 让 CUDA 的内存分配器稳定，并确保 Triton Autotune 完全结束
    s = torch.cuda.Stream()
    s.wait_stream(torch.cuda.current_stream())
    with torch.cuda.stream(s), torch.inference_mode():
        # 增加 Warmup 次数至 10-20 次，确保 Triton 的 JIT 编译和 autotune 彻底跑完
        for i in range(15): 
            static_out_triton = triton_model(static_X_q, static_X_kv)
    
    torch.cuda.current_stream().wait_stream(s)
    
    # 🌟 关键修复: 在这里强制同步 GPU，拦截 Warmup 阶段可能发生的异步错误！
    # 如果 Triton 算子本身有 Bug (如显存越界)，报错会在这里抛出，而不是在 capture_end 抛出。
    torch.cuda.synchronize() 
    print("✅ Warmup 成功，未发现底层算子错误，准备 Capture...")
    ...
    try:
        ...
        print(f"❌ CUDA Graph 录制过程中发生同步阻断: {e}")
        raise
