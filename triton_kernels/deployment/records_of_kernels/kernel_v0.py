...
def batched_flash_decoding_mha_phase1(
    ...
    # 🔥 核心修正：定义一个 16 的伪造维度，用于欺骗 Tensor Core
    # ... 前面的代码不变 ...
    offs_q_fake = tl.arange(0, 16)
    mask_q_fake = offs_q_fake < 1
    ...
    if chunk_start < chunk_end:
        ...
        q_2d = tl.where(mask_q_fake[:, None], q[None, :], 0.0)

        # 🔥 核心修正 1：把 acc 也变成 2D 矩阵，并在整个循环中保持！
        acc_2d = tl.zeros([16, BLOCK_D], dtype=tl.float32)
        ...
        for start_n in range(chunk_start, chunk_end, BLOCK_N):
            ...
            qk_2d = tl.dot(q_2d.to(tl.float16), tl.trans(k.to(tl.float16)))
            qk = tl.sum(qk_2d, axis=0) # qk 的 1D 降维暂时保留，因为 m_i 逻辑需要
            ...
            p_2d = tl.where(mask_q_fake[:, None], p[None, :], 0.0)
            
            # 🔥 核心修正 3：直接累加到 2D 矩阵上，去掉了循环里的 tl.sum！
            acc_2d += tl.dot(p_2d.to(tl.float16), v.to(tl.float16))

            m_i = m_ij
            l_i = l_i * alpha + l_ij

        # 🔥 核心修正 4：等整个循环跑完，在循环外部做最后一次降维！
        acc = tl.sum(acc_2d, axis=0)
...
def batched_flash_decoding_mha(q, k, v, q_cluster_ids, k_cu_seqlens, num_splits=16):
    ...
    batched_flash_decoding_mha_phase1[(N_q, num_splits, B * H)](
        ...
        actual_d=D, H=H, BLOCK_N=32, BLOCK_D=BLOCK_D, num_warps=2
    )
    ...
    batched_flash_decoding_mha_phase2[(N_q, B * H)](
        ...
        actual_d=D, H=H, NUM_SPLITS=num_splits, BLOCK_D=BLOCK_D, num_warps=2
    )
    return out, LSE
...
def triton_attention_backward_split_mha(Q_sorted, K_sorted, V_sorted, Out_sorted, dO_sorted, 
    ...
    # 替换前向与反向中的对应路由计数逻辑
    counts = F.one_hot(q_cluster_ids.to(torch.int64), num_classes=M).sum(dim=1) # 假设 cluster_ids 是 2D (B, N)
    q_cu_seqlens[:, 1:] = torch.cumsum(counts, dim=1)
...
class TorchCrossMoEMultiHeadAttention(nn.Module):
    ...
    def forward(self, X_q: torch.Tensor, X_kv: torch.Tensor):
        ...
        scores = torch.matmul(q, k.transpose(-2, -1)) * sm_scale # [B, H, N_q, N_k]
        
        # 将不属于同一聚类的 score 设为负无穷
        scores = scores.masked_fill(~mask, float('-inf'))

        # 4. 计算 Softmax
        attn = torch.softmax(scores, dim=-1)
        
        # 【关键点】处理原代码中 `if not mask.any(): continue` 的情况
        # 如果某一行全是 -inf (即该 query 找不到同 cluster 的 key)，softmax 会产生 NaN。
        # 我们使用 nan_to_num 将这些 NaN 转为 0，这与原代码直接留空的 out 矩阵保持一致。
        attn = torch.nan_to_num(attn, nan=0.0)

        # 5. 乘以 Value 得到最终结果
        out = torch.matmul(attn, v) # [B, H, N_q, d_out_head]
