...
class Dataset_Custom_uni_modal(Dataset):
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.args.features == "S":
            self.start_date=df_start_date[border1:border2].values
            self.end_date=df_end_date[border1:border2].values
        self.text = df_text[border1:border2].values
    ...
    def get_full_text(self):
        """
        将所有文本值去重后拼接成一个完整的长段落

        参数:
        texts -- 文本列表，每个元素是一个包含多个句子的字符串

        返回:
        full_text -- 去重后拼接而成的完整长段落
        """
        prefix = "Available facts are as follows:"
        seen_sentences = set()
        unique_sentences = []
        
        border1 = 0
        border2 = int(len(self.df_text) * 0.7)
        texts = self.df_text[border1:border2].values
        # print(len(texts))
        # print(type(texts))
        # texts = str(texts)
        ...
        # 拼接成完整段落
        full_text = prefix + " " + "; ".join(unique_sentences) + ";"
        return full_text
...
class Dataset_M4(Dataset):
    ...
    def __read_data__(self):
        ...
        training_values = np.array(
            [v[~np.isnan(v)] for v in dataset.values[dataset.groups == self.seasonal_patterns]])  # split different frequencies
