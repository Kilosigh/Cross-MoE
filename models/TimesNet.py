...
def FFT_for_Period(x, k=2):
    ...
    frequency_list = torch.hypot(xf.real, xf.imag).mean(0).mean(-1)
    frequency_list[0] = 0
    _, top_list = torch.topk(frequency_list, k)
    top_list = top_list.detach().cpu().numpy()
    period = x.shape[1] // top_list
    return period, torch.hypot(xf.real, xf.imag).mean(-1)[:, top_list]
...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, x_mark):
        ...
        x = self.dropout(x)
        if self.predict_linear is not None:
            x = self.predict_linear(x.permute(0, 2, 1)).permute(0, 2, 1)
        return x

class Encoder(nn.Module):
    def __init__(self, model, layer, norm_layer=None):
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.enc_embedding = DataEmbedding(configs.enc_in, configs.d_model, configs.embed, configs.freq,
                                           configs.dropout, configs=configs)
        self.layer = configs.e_layers
        self.layer_norm = nn.LayerNorm(configs.d_model)
        self.encoder = Encoder(self.model, self.layer, norm_layer=self.layer_norm)
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            self.head = nn.Linear(
