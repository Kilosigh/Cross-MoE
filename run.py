...
from exp.exp_long_term_forecasting_MM import Exp_Long_Term_Forecast_MM
from exp.exp_long_term_forecasting_MoE import Exp_Long_Term_Forecast_MoE
...
if __name__ == '__main__':
    ...
    parser.add_argument('--learning_rate4', type=float, default=5e-4, help='moe learning rate')
    parser.add_argument('--learning_rate5', type=float, default=1e-3, help='eva learning rate')
    ...
    # use_MoE
    parser.add_argument('--use_tx_moe', type=int, default=0)
    parser.add_argument('--use_ts_moe', type=int, default=0)
    parser.add_argument('--num_tx_experts', type=int, default=0)
    parser.add_argument('--num_ts_experts', type=int, default=0)
    parser.add_argument('--num_tx_moe_layers', type=int, default=1)
    parser.add_argument('--num_ts_moe_layers', type=int, default=0)
    parser.add_argument('--is_scale_prob', type=int, default=1)
    parser.add_argument('--drop_tokens', type=int, default=0)
    ...
    # 0 denotes disabled, 1 denotes mlp, 2 denotes cosine sim 
    parser.add_argument('--use_eva', type=int, default=0)
    parser.add_argument('--use_Cross_MoE', type=int, default=0)

    parser.add_argument('--use_pretrained_ts_model', type=int, default=0)
    # parser.add_argument('--text_top_k', type=int, default=3)
    parser.add_argument('--mix_type', type=int, default=0, help='0 for addition, 1 for crossattn, 2 for MoE-Enhanced attn')
    parser.add_argument('--use_Cross_ranker', type=int, default=0, help='whether use the Cross Ranker')
    parser.add_argument('--use_trainable_center', type=int, default=0, help='whether to use trainable cluster center')
    parser.add_argument('--use_k_means_init', type=int, default=1, help='whether to use k-means to init the cluster center.')
    
    
    parser.add_argument('--calculate_overhead', type=int, default=0, help='whether print the overhead of the model')

    # visualization
    parser.add_argument('--plot_attn', type=int, default=0, help='whether to plot attn')
    parser.add_argument('--plot_tsne', type=int, default=1, help='whether to plot t-SNE figure in mix_type 2')
    parser.add_argument('--is_testing', type=int, default=1, help='Denote whether the exp is running test')

    # distributed training
    parser.add_argument('--use_dist_training', type=int, default=0, help='whether to use distributed training')
    ...
    args.cluster_init_text_data = None
    args.sampler = None
    ...
    if args.task_name == 'long_term_forecast':
        if args.use_Unified_model or args.use_Cross_MoE:
            Exp = Exp_Long_Term_Forecast_MM
        else:
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            args.setting = get_setting_string(args,ii)

            print('>>>>>>>start training : {}>>>>>>>>>>>>>>>>>>>>>>>>>>'.format(args.setting))
            exp.train(args.setting)

            if args.calculate_overhead:
                exit()

            print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(args.setting))
            now_mse=exp.test(args.setting, test=1)
            torch.cuda.empty_cache()
    else:
        args.setting = get_setting_string(args,0)
        exp = Exp(args)  # set experiments
        print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(args.setting))
        now_mse=exp.test(args.setting, test=1)
