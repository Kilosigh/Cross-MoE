...
class DynamicAuxLossWeight:
    def __init__(self, alpha=0.01, min_weight=0.0001, max_weight=0.1):
        self.alpha = alpha
...
class Exp_Long_Term_Forecast_MoE(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_MoE, self).__init__(args)
        configs=args
        ...
        self.use_ts_moe = self.args.use_ts_moe
        self.use_tx_moe = self.args.use_tx_moe
        if self.use_tx_moe == True:
            tx_dim = self.args.llm_dim
            num_experts= self.args.num_tx_experts# Define the number of experts for each MoE layer
        ...
        if self.use_ts_moe == True:
            ts_dim = self.args.c_out
            num_experts = self.args.num_ts_experts # Define the number of experts for each MoE layer
        ...
        if self.args.use_eva:
            self.prompt_weight_method = Modal_eval(self.args.use_eva)
    ...
    def _select_optimizer_ts_moe(self):
        model_optim = optim.Adam(self.ts_moe.parameters(), lr=self.args.learning_rate4)
        return model_optim
    
    def _select_optimizer_tx_moe(self):
        model_optim = optim.Adam(self.tx_moe.parameters(), lr=self.args.learning_rate4)
        return model_optim
    
    def _select_optimizer_eva(self):
        model_optim = optim.Adam(self.eva.parameters(), lr=self.args.learning_rate5)
        return model_optim
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                if self.use_ts_moe:
                    outputs, aux_loss_ts = self.ts_moe(outputs)
                    outputs = self.moe_ts_dropout(outputs)
                ...
                loss = loss_ts
                if self.args.use_text and self.args.use_tx_moe:
                    loss += dynamic_weight(loss_ts, aux_loss_tx).detach() * aux_loss_tx

                if self.args.use_ts_moe:
                    loss += dynamic_weight(loss_ts, aux_loss_ts).detach() * aux_loss_ts
    ...
    def train(self, setting):
        ...
        if self.use_tx_moe:
            model_optim_tx_moe = self._select_optimizer_tx_moe()
        if self.use_ts_moe:
            model_optim_ts_moe = self._select_optimizer_ts_moe()
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(train_loader):
                ...
                if self.use_tx_moe:
                    model_optim_tx_moe.zero_grad()
                if self.use_ts_moe:
                    model_optim_ts_moe.zero_grad()
                ...
                if self.args.use_amp:
                    ...
                    if self.args.use_ts_moe:
                        model_optim_ts_moe.step()
                    if self.args.use_tx_moe:
                        model_optim_tx_moe.step()
    ...
    def test(self, setting, test=0):
        ...
        for i, sublist in enumerate(preds):
            print(f"Length of sublist {i}: {len(sublist)}")
